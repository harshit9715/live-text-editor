...
const Notifications = () => {
  ...
  return (
    ...
    {count > 0 && (<div className="absolute right-2 top-2 z-20 size-2 rounded-full bg-blue-500"></div>)}
      </PopoverTrigger>
      <PopoverContent align="end" className="shad-popover">
        <LiveblocksUIConfig overrides={{INBOX_NOTIFICATION_TEXT_MENTION: (user: ReactNode) => {
            return <>{user} mentioned you in a document</>;}
        }}></LiveblocksUIConfig>
        <InboxNotificationList>
            ...
            {unreadNotifications.length > 0 && (unreadNotifications.map((notification) => (
                ...
                kinds={{
                    ...
                    showRoomName={false}
                    />,
                    textMention: (props) => <InboxNotification.TextMention {...props}
                    showRoomName={false}
                    />,
                    $documentAccess: (props) => <InboxNotification.Custom {...props} title={props.inboxNotification.activities[0].data.title} aside={<InboxNotification.Icon className="bg-transparent">
